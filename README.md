...
- 📤 **站内上传**：上传封面图片（≤ 25MB）+ 音频（MP3/WAV，≤ 100MB），通过 GitHub API 写入仓库
...
- 上传受 GitHub API 限制：音频单文件 ≤ 100MB、封面 ≤ 25MB；但 API 实际处理超大约 50MB 的大文件经常报错（422 too large），**强烈建议使用 MP3**（WAV 体积通常大 5-10 倍，转成 320kbps MP3 后一首歌仅约 5-10MB）
